...
[FromBody] TicketTbl Model
        ...
        [HttpPost]
        [Route("GetMainAdmin")]
        public async Task<ActionResult<TicketTbl>> GetTicketMainAdmin([FromQuery(Name = "page")] int page, [FromQuery(Name = "pageSize")] int pageSize, string search, [FromBody] TicketTbl Model)
        {

            var tickets = await _ticketRepository.GetTicketMainAdmin(page == 0 ? 1 : page, pageSize == 0 ? 10 : pageSize, search, Convert.ToInt32(Model.MainAdminId));
            if (tickets == null)
            {
                return BadRequest();
            }
